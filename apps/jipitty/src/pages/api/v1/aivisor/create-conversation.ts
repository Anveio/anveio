...
export default async function sendChatMessage(request: NextRequest) {
	...
	if (!conversationId) {
		...
		conversationId = await createConversationFromScratch(email, "private")
	}
	...
	console.log(
		"Found messages for conversation wit ID ",
		conversationId,
		"--",
		messageHistory
	)
