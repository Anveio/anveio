...
import { OpenAIAPIEndpoints, OpenAIEdgeClientDefinition } from "./types";
 ...
 */
export const OpenAIEdgeClient: OpenAIEdgeClientDefinition = async (
  endpoint,
  args,
  { mode = "tokens", apiKey = process.env.OPENAI_API_KEY, controller } = {}
) => {
  if (!apiKey) {
