...
USER_NAME="anveio-post-artifacts-prod"
AWS_API_QUERY='AccessKey.[AccessKeyId,SecretAccessKey]'

print_usage() {
  cat <<'USAGE'
Usage: ./rotate-post-artifacts-prod-key.sh [--deactivate OLD_ACCESS_KEY_ID]
...
Next steps:
  ...
  2. Update Convex production env vars:
       npx convex env set prod AWS_ACCESS_KEY_ID "$ACCESS_KEY_ID"
       npx convex env set prod AWS_SECRET_ACCESS_KEY "$SECRET_ACCESS_KEY"
