...
): payload is Extract<FragmentPayload, { kind: 'text' }> => payload.kind === 'text'
...
const buildTextPayload = (text: string): Extract<FragmentPayload, { kind: 'text' }> => ({
  kind: 'text',
  version: '1',
  editorState: text,
  ...
  const handleSave = () => {
    startTransition(async () => {
      try {
        ...
        console.error(cause)
        setMessage(cause instanceof Error ? cause.message : 'Failed to save fragment')
      }
    })
  }
  ...
  const handleDelete = () => {
    startTransition(async () => {
      try {
        ...
        console.error(cause)
        setMessage(cause instanceof Error ? cause.message : 'Failed to delete fragment')
      }
    })
  }
  ...
  const handleMove = (direction: 'up' | 'down') => {
    startTransition(async () => {
      try {
        ...
        console.error(cause)
        setMessage(cause instanceof Error ? cause.message : 'Failed to reorder fragment')
      }
    })
  }
...
export function PostEditor({ postId }: PostEditorProps) {
  ...
  if (data === null) {
    return <p className="text-sm text-red-600 dark:text-red-400">Post not found.</p>
  }
  ...
  const handleMetadataSave = () => {
    startMetadataTransition(async () => {
      try {
        ...
        console.error(cause)
        setFeedback(cause instanceof Error ? cause.message : 'Failed to save metadata')
      }
    })
  }
  ...
  const handleAddFragment = () => {
    ...
    addDraftFragment({
      ...
      .catch((cause) => {
        console.error(cause)
        setFeedback(cause instanceof Error ? cause.message : 'Failed to add fragment')
      })
  }
  ...
  const handlePublish = () => {
    startPublishTransition(async () => {
      try {
        ...
        console.error(cause)
        setFeedback(cause instanceof Error ? cause.message : 'Failed to publish post')
      }
    })
  }
  ...
  return (
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-8 py-8 md:py-12">
      <section className="rounded-2xl border border-slate-200/70 bg-white/90 p-6 shadow-sm dark:border-slate-700/60 dark:bg-slate-900/70">
        <header className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400">Post ID: {post.publicId}</p>
          </div>
        ...
        <div className="mt-6 grid gap-4">
          ...
          <label className="flex flex-col gap-2 text-sm">
            <span className="font-semibold text-slate-700 dark:text-slate-200">Slug</span>
            <input
              type="text"
              value={metadata.slug}
              onChange={(event) => setMetadata((prev) => ({ ...prev, slug: event.target.value }))}
              className="w-full rounded-lg border border-slate-300 bg-white/95 px-3 py-2 text-base text-slate-900 shadow-sm focus:border-slate-500 focus:outline-none focus:ring-2 focus:ring-slate-200 dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-100"
            />
          </label>
          <label className="flex flex-col gap-2 text-sm">
            ...
            <textarea
              ...
              onChange={(event) => setMetadata((prev) => ({ ...prev, summary: event.target.value }))}
              className="w-full rounded-lg border border-slate-300 bg-white/95 px-3 py-2 text-base text-slate-900 shadow-sm focus:border-slate-500 focus:outline-none focus:ring-2 focus:ring-slate-200 dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-100"
            />
          </label>
          <button
      ...
      <section className="rounded-2xl border border-slate-200/70 bg-white/90 p-6 shadow-sm dark:border-slate-700/60 dark:bg-slate-900/70">
        <header className="flex flex-wrap items-center justify-between gap-4">
          <h2 className="text-xl font-semibold text-slate-900 dark:text-slate-100">Draft fragments</h2>
        ...
        <div className="mt-4 flex flex-col gap-4 md:flex-row">
          ...
          <div className="flex w-full flex-col gap-3 md:w-56">
            <label className="flex flex-col gap-2 text-sm">
              <span className="font-semibold text-slate-700 dark:text-slate-200">Priority</span>
              <select
      ...
      <section className="rounded-2xl border border-slate-200/70 bg-white/90 p-6 shadow-sm dark:border-slate-700/60 dark:bg-slate-900/70">
        <header className="flex flex-wrap items-center justify-between gap-4">
          <h2 className="text-xl font-semibold text-slate-900 dark:text-slate-100">Publish</h2>
          <button
            type="button"
        ...
        {feedback && (
          <p className="mt-3 text-sm text-slate-600 dark:text-slate-300">{feedback}</p>
        )}
        <div className="mt-6">
          ...
          {publishHistory.length === 0 ? (
            <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">No publish events yet.</p>
          ) : (
