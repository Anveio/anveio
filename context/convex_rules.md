...
* Always include all index fields in the index name. For example, if an index is defined as `["field1", "field2"]`, the index name should be "by\_field1\_and\_field2".
...
const messages = await ctx.db .query("messages") .withSearchIndex("search\_body", (q) => q.search("body", "hello hi").eq("channel", "#general"), ) .take(10);
...
Write your Convex functions in any file inside the `./convex` directory
